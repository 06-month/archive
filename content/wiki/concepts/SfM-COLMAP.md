...
**Structure-from-Motion(SfM)**: 여러 장의 2D 이미지로부터 **카메라 포즈(외부·내부 파라미터)** 와 장면의 **희소 3D 점군(sparse point cloud)** 을 동시에 추정하는 고전 컴퓨터비전 파이프라인. Radiance Field NVS 연구의 **표준 전처리**.
...
## Radiance Field 연구에서의 위치 (공통 개념인 이유)
...
- [[Radiance Field-Volume Rendering]] — SfM 산출물이 Radiance Field 최적화의 입력
