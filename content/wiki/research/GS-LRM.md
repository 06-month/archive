...
- **계보 형제**: LRM·Instant3D(triplane NeRF LRM, 본 논문이 GS로 대체)·[[pixelSplat]](본 논문이 +2.2dB로 능가, 프로토콜 차용)·LGM(concurrent pixel-aligned GS). [[MVSplat]] — 같은 sparse-view feed-forward GS·동일 RealEstate10K 벤치마크지만 **cost volume 매칭**으로 차별(본 논문은 self-attention 회귀). [[VGGT]]·[[DUSt3R]] — feed-forward 복원의 다른 분기(pointmap·카메라까지 예측).
