...
- **계보 형제**: LRM·Instant3D(triplane NeRF LRM, 본 논문이 GS로 대체)·[[pixelSplat]](본 논문이 +2.2dB로 능가, 프로토콜 차용)·[[LGM]](concurrent pixel-aligned GS — U-Net+멀티뷰 확산으로 **객체 생성** 분기).
- **스케일업 후계**: [[Long-LRM]] — 뷰·범위 확장(32뷰 광범위, Mamba2 하이브리드) / [[BTimer]] — 시간 확장(bullet-time 동적). 둘 다 본 노트의 per-pixel GS·트랜스포머 정식화를 직접 계승. [[MVSplat]] — 같은 sparse-view feed-forward GS·동일 RealEstate10K 벤치마크지만 **cost volume 매칭**으로 차별(본 논문은 self-attention 회귀). [[VGGT]]·[[DUSt3R]] — feed-forward 복원의 다른 분기(pointmap·카메라까지 예측).
