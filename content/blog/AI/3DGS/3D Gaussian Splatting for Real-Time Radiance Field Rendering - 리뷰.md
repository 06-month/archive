...
---
#### 3.2 2D Gaussian Footprint 생성

위의 3.1 섹션에서 3D Gaussian의 중심과 공분산을 이미지 평면으로 투영하였다.

이 때, 이미지 평면 위에 가우시안이 차지하는 영역을 2D Gaussian Footprint라고 한다.

Footprint의 중심은 projection된 Gaussian의 중심이며, 모양은 2D 공분산 $\Sigma’$에 의해 결정된다.

이는 즉 아래를 뜻한다.
- 공분산이 작으면 작은 Footprint를 가진다.
- 공분산이 크면 넓은 Footprint를 가진다.
- 비등방적인 공분산이면 원이 아닌 타원 형태의 Footprint가 생성된다.

하지만 Gaussian 함수는 수학적으로 무한히 퍼지는 함수이다.

따라서 실제 렌더링에서는 Gaussian이 영향을 줄 수 있는 화면상의 범위를 계산한 뒤 그 범위 안의 픽셀만 평가한다.

즉 하나의 Gaussian은 이미지 전체에 영향을 주는 것이 아닌, 자신의 Footprint 안에 존재하는 픽셀들에만 영향을 준다.

이렇게 생성된 Footprint는 이후 어떤 타일에 포함되는지 계산하는 데 사용된다.

---
#### 3.3 Tile-based Binning
...
Tile-based Binning을 통해 각 tile이 처리해야 할 Gaussian 목록이 정해지면, 다음으로 해당 Gaussian들을 depth 기준으로 정렬한다.
...
#### 3.5 Gaussian Splatting / Pixel-wise Gaussian Evaluation
...
각 Gaussian은 자신의 Footprint 안에 있는 모든 픽셀에 동일하게 영향을 주지 않는다.

투영한 Gaussian 중심에 가까운 픽셀은 큰 영향을 받고, 중심에서 멀어질수록 영향은 작아진다.

이를 계산하기 위해 이미지 평면의 픽셀 위치 $p$와 2D Gaussian 중심 $\mu’$ 사이의 거리를 사용한다.

이 때, 단순한 유클리드 거리가 아닌, 2D 공분산 $\Sigma’$를 반영한 거리로 Gaussian weight를 계산한다.
$$G_{2D}
(p) = \exp(-\frac{1}{2} \cdot (p - μ')^T Σ'^{-1} (p - μ'))$$
- $p = (x, y)$
- $\mu’ = (u, v)$

여기서 $G_{2D}(p)$는 해당 Gaussian이 픽셀 p에 얼마나 강하게 영향을 주는지를 나타낸다.

위 과정을 통해 각 가우시안들이 Footprint 내의 범위에서 영향을 주는 정도를 계산할 수 있다.

이렇게 계산된 Gaussian weight는 다음 Alpha Blending 단계에서 각 Gaussian의 opacity와 결합되어, 해당 Gaussian이 픽셀 색에 얼마나 기여할지를 결정한다.


---

#### 3.6 Alpha Blending을 통한 픽셀 색 계산

3DGS에서 여러 가우시안들이 같은 픽셀에 영향을 줄 수 있다.

이때, 앞쪽 가우시안과 뒷쪽 가우시안을 depth 순서에 따라 합성하고, 3DGS는 alpha blending을 사용해 구현한다.

최종 픽셀 색은 다음처럼 계산할 수 있다.$$C = \sum_{i=1}^{N} T_i \alpha'_i \mathbf{c}_i, T_i = \prod_{j=1}^{i-1} (1-\alpha'_j)$$
- $c_i$: $i$번째 가우시안의 색
- $\alpha'_i$: i번째 Gaussian이 해당 픽셀에서 실제 blending에 사용하는 pixel-wise alpha
- $T_i$: $i$번째 가우시안 앞쪽까지의 transmittance

여기서 $\alpha'_i$는 단순히 Gaussian 자체의 불투명도만을 의미하는 것이 아니라, 앞 절에서 계산한 픽셀별 Gaussian의 weight가 반영된 값이다.

즉, 픽셀 $p$에 대해 $\alpha'_i(p)$는 Gaussian의 불투명도와 $G_{2D}(p)$를 곱해 계산된다. $$α'_i(p) = \alpha_i × G_{2D,i}(p)$$
- $\alpha_i$: $i$번째 가우시안의 불투명도

---

#### 3.7 왜 Differentiable한가

앞의 과정을 통해 3DGS는 3D Gaussian을 이미지 평면에 투영하고, 각 픽셀에서 Gaussian의 weight와 alpha blending을 통해 최종 렌더링 이미지를 생성하였다.

여기서 중요한 점은 이 렌더링 과정이 단순히 이미지를 생성하는 것에 그치지 않고, loss의 gradient를 다시 Gaussian의 parameter로 전달할 수 있다는 점이다.

렌더링 된 이미지를 $\hat{I}$, 실제 이미지를 $I$라고 하면, 두 이미지 사이의 Loss를 계산할 수 있다.

본 논문에서는 렌더링 이미지와 실제 RGB 이미지의 픽셀 차이를 측정하기 위해 L1 Loss를 사용한다.

$$L_1(\hat{I}, I) = \frac{1}{N} \sum_{p=1}^{N} \left| \hat{I}(p)-I(p) \right|$$

여기서 $\hat{I}$는 각 Gaussian의 위치, 크기, 회전, 불투명도, 색상 계수에 의해 결정된다.

즉, $\hat{I}=R(\mu, s, q, \alpha, SH)$ 으로 볼 수 있다.

따라서 Loss의 gradient는 렌더링 과정을 거꾸로 따라가며 다음 Gaussian의 파라미터들로 전달된다.

즉, 3DGS의 rasterizer가 differentiable하다는 것은 렌더링 이미지와 실제 이미지 사이의 차이를 이용해, 각 Gaussian들의 parameter를 경사하강법으로 직접 업데이트 할 수 있다는 의미이다.

일반적인 rasterization이 주로 주어진 geometry를 화면에 그려내는 과정이라면, 3DGS의 differentiable rasterization은 렌더링 결과의 오차를 다시 Gaussian parameter 최적화에 사용할 수 있도록 설계된 렌더링 과정이다.
